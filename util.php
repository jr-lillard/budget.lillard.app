...
function budget_is_client_account(PDO $pdo, int $accountId): bool
{
    ...
    $stmt = $pdo->prepare('SELECT IFNULL(is_client, 0) FROM accounts WHERE id = ? LIMIT 1');
...
function budget_create_client_payment(
    ...
    if (!is_numeric($amount)) {
        throw new RuntimeException('Client payment amount format invalid.');
    }
    $paymentAmount = round(abs((float)$amount), 2);
    if ($paymentAmount <= 0.0) {
        throw new RuntimeException('Client payment amount must be greater than zero.');
    }
